...
Cell::Cell(const unsigned char* cellBuffer, const BtreePageType type)
    : id(0)
    , record(*this)
{
    // Size of cell data
    uint32_t offset = 0;
    auto [cellSize, cellSizeB] = VLInt::parse(cellBuffer);
    offset += cellSizeB;

    // Different types define cell header fields
    if (type == BtreePageType::TableLeaf) {
        // Get Record ID
        auto [cellId, cellIdB] = VLInt::parse(cellBuffer + offset);
        offset += cellIdB;
        id = cellId;
        if constexpr (DEBUG)
