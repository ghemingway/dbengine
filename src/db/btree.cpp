...
BTreePage *BTreePage::read(std::iostream &stream, const uint32_t pageSize,
                           bool pageZero) {
  char buffer[pageSize];
  stream.read(buffer, pageSize);
  switch (const auto *header = reinterpret_cast<BTreeHeader *>(buffer);
          header->type) {
  case TableLeaf:
    return new BTreeTableLeaf(buffer, pageSize, pageZero);
  case TableInterior:
    return new BTreeTableInterior(buffer, pageSize, pageZero);
  case IndexLeaf:
    return new BTreeIndexLeaf(buffer, pageSize, pageZero);
  case IndexInterior:
    return new BTreeIndexInterior(buffer, pageSize, pageZero);
...
BTreePage::BTreePage(const char *pageBuffer, bool pageZero)
    : Page(0x0D), header{}, cells() {
...
uint32_t BTreePage::numCells() const { return cells.size(); }
...
void BTreePage::parseCells(const char *pageBuffer, const bool pageZero) {
  // Parse cells
  uint16_t cellOffset;
  for (uint32_t i = 0; i < header.numCells; ++i) {
    const auto offset =
        i * 2 + (header.type == BtreePageType::TableInterior ? 12 : 8);
...
BTreeTableLeaf::BTreeTableLeaf(const char *pageBuffer, const uint32_t,
                               bool pageZero)
...
BTreeTableInterior::BTreeTableInterior(const char *pageBuffer, const uint32_t,
                                       bool pageZero)
    : BTreePage(pageBuffer, pageZero) {
...
BTreeIndexLeaf::BTreeIndexLeaf(const char *pageBuffer, const uint32_t,
                               bool pageZero)
    : BTreePage(pageBuffer, pageZero) {
...
BTreeIndexInterior::BTreeIndexInterior(const char *pageBuffer, const uint32_t,
                                       bool pageZero)
    : BTreePage(pageBuffer, pageZero) {
